...
let trendingTokens: { symbol: string; mint: string }[] = [
...
async function fetchImageFromUri(uri: string, retries = 3): Promise<string> {
...
async function updateMetadataCache() {
    ...
    metadataList.forEach(meta => {
        if (meta && meta.account) metadataCache[meta.account] = meta;
    });
}
...
export const getTrendingTokensWithDetails = async (): Promise<TrendingToken[]> => {
...
export const updateTrendingTokens = (newList: { symbol: string; mint: string }[]) => {
    trendingTokens = newList;
    updateMetadataCache(); // Refresh cache
};

export const getTrendingTokensList = () => trendingTokens;
