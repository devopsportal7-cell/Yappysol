...
exports.TokenCreationService = exports.tokenCreationSessions = void 0;
    ...
    try {
        ...
        if (!transaction || !transaction.meta) {
            ...
            try {
                ...
                const solscanResponse = await (0, node_fetch_1.default)(`https://public-api.solscan.io/transaction/${signature}`);
    ...
    switch (step) {
        ...
        case 'symbol':
            if (!/^[A-Z0-9]{2,10}$/.test(input))
                return 'Ticker must be 2-10 uppercase letters or numbers.';
            return null;
        case 'amount':
            if (input === undefined || isNaN(Number(input)) || Number(input) < 0)
                return 'Amount must be zero or a positive number.';
            // For Bonk, provide guidance about minimum requirements
            if (input === 0)
                return 'Amount must be greater than 0. For Bonk pool, minimum 0.01 SOL is recommended.';
            return null;
        case 'description':
        ...
        case 'website':
            ...
            return null;
        case 'pool':
            if (!input || !['pump', 'bonk'].includes(input.toLowerCase()))
                return 'Pool must be either "pump" or "bonk".';
            return null;
        default:
            return null;
    }
}
async function createTokenMetadata(name, symbol, description, imageFile, twitter, telegram, website, pool) {
    if (pool === 'pump') {
        ...
        const response = await (0, node_fetch_1.default)("https://pump.fun/api/ipfs", {
            method: "POST",
            body: formData,
        });
        ...
        const imageUri = await (0, pinata_1.uploadImageToIPFS)(imageFile);
        ...
        const response = await (0, node_fetch_1.default)("https://nft-storage.letsbonk22.workers.dev/upload/meta", {
            method: "POST",
            headers: {
...
async function createTokenTransaction(publicKey, tokenMetadata, mint, amount, pool) {
    ...
    if (pool === 'bonk') {
        try {
            ...
            const walletResponse = await (0, node_fetch_1.default)("https://pumpportal.fun/api/create-wallet", {
                method: "GET",
            });
    ...
    if (pool === 'bonk' && apiKey) {
        ...
        try {
            const walletInfoResponse = await (0, node_fetch_1.default)(`https://pumpportal.fun/api/wallet-info?api-key=${apiKey}`);
    ...
    if (pool === 'pump') {
        ...
        try {
            ...
            const solscanResponse = await (0, node_fetch_1.default)(`https://public-api.solscan.io/transaction/${data.signature}`);
            if (solscanResponse.status === 404) {
                console.log('[DEBUG] Transaction not found on Solscan - waiting and retrying...');
                // Wait a bit more and retry
                await new Promise(resolve => setTimeout(resolve, 3000));
                const retryResponse = await (0, node_fetch_1.default)(`https://public-api.solscan.io/transaction/${data.signature}`);
...
class TokenCreationService {
    ...
    async handleCreationIntent(message, context) {
        ...
        let session = exports.tokenCreationSessions[userId] || { step: null };
            ...
            if (userInput.trim().toLowerCase() === 'yes') {
                ...
                switch (session.step) {
                    ...
                    case 'twitter':
                        ...
                        break;
                    case 'telegram':
                        prompt = 'Telegram link? (must be a valid URL, or type "skip" to leave blank)';
                        break;
                    case 'website':
                        prompt = 'Website? (must be a valid URL, or type "skip" to leave blank)';
                        break;
                    case 'pool':
                    ...
                    case 'confirmation':
                        ...
                        break;
                    default:
                        prompt = 'Please continue the token creation process.';
                        break;
                }
                ...
                case 'confirmation': {
                    ...
                    if (session.pool === 'pump') {
                        ...
                        try {
                            ...
                            if (walletInfo) {
                                fees = WalletService_1.WalletService.calculateTransactionFees('token-creation', session.amount);
                                summary += `\n💰 **Transaction Details:**\n` +
    ...
    getSession(userId) {
        return exports.tokenCreationSessions[userId] || null;
    }
    ...
    async createToken(params) {
        ...
        const walletInfo = await WalletService_1.WalletService.getUserDefaultWallet(params.userId);
        if (!walletInfo) {
        ...
        try {
            ...
            if (params.pool === 'pump') {
                const fees = WalletService_1.WalletService.calculateTransactionFees('token-creation', params.amount);
                const balanceCheck = await WalletService_1.WalletService.hasSufficientBalance(walletInfo.id, params.amount, fees);
                if (!balanceCheck.sufficient) {
                    throw new Error(`Insufficient balance. Need ${balanceCheck.required?.toFixed(6)} SOL, have ${balanceCheck.currentBalance.toFixed(6)} SOL. ` +
            ...
            const metadataUri = await createTokenMetadata(params.name, params.symbol, params.description, params.imageFile, params.twitter || '', params.telegram || '', params.website || '', params.pool);
            ...
            const result = await createTokenTransaction(params.pool === 'pump' ? walletInfo.publicKey : '', // Only use user's wallet for Pump.fun
            {
                name: params.name,
            ...
            if (result.unsignedTransaction) {
                try {
                    ...
                    const connection = new web3_js_1.Connection(heliusRpcUrl);
            ...
            if (result.signature) {
                ...
                if (result.error) {
                    ...
                    setTimeout(async () => {
                        try {
                            if (result.mint) {
                                await TokenPriceTrackingService_1.tokenPriceTrackingService.getTokenPrice(result.mint);
                            }
                        }
                        catch (error) {
                            console.error('[TokenCreationService] Error starting price tracking:', error);
...
exports.TokenCreationService = TokenCreationService;
